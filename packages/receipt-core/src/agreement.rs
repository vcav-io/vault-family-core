    ...
    fn test_agreement_hash_fixtures() {
        ...
        for (i, fixture) in fixtures.iter().enumerate() {
            let fields: SessionAgreementFields =
                serde_json::from_value(fixture["fields"].clone())
                    .unwrap_or_else(|e| panic!("Failed to parse fixture {} fields: {}", i, e));
            let expected = fixture["expected_hash"]
                .as_str()
                .unwrap_or_else(|| panic!("Fixture {} missing expected_hash", i));
            ...
            assert_eq!(
                ...
                i,
                fixture.get("name").and_then(|n| n.as_str()).unwrap_or("unnamed")
            );
        }
    }
}
